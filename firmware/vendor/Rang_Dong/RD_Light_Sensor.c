...
void Lux_send(unsigned int threshold){
	unsigned int luxRawData = RD_Light_Sensor_read_raw_val();
	real_lux_new = CalculateLux(luxRawData);
...
void RD_Send_raw_Lux(u16 Lux_raw_val,unsigned int lux_real)
{
	...
	*(light_sensor_to_gw_tx_buff+5) = (u8)(lux_real);
	mesh_tx_cmd2normal_primary(SENSOR_STATUS, (u8 *)light_sensor_to_gw_tx_buff, 6, rd_box_addr, 2);
}
...
void lightSensorSenData(unsigned int lux){
	unsigned char buffSend[6] = {0x04,0x00,0x00,0x00,0x00,0x00};
	buffSend[2] = lux>>8;
	buffSend[3] = lux;
	mesh_tx_cmd2normal_primary(SENSOR_STATUS, (u8 *)buffSend,6, rd_box_addr, 2);
}
