...
#include "vendor/Rang_Dong/RD_Pir_AC_Control.h"
#include "vendor/Rang_Dong/RD_type_device.h"
#include "vendor/Rang_Dong/RD_control.h"
#include "vendor/Rang_Dong/SceneControl.h"
...
_attribute_ram_code_ int main (void)    //must run in ramcode
{
	...
	#endif
	uart_CSend("pir start\n");
	check_prov_status_old = is_provision_success();
	pir_intput_init();
	btn_init();
	led_init();
	Light_sensor_i2c_init();
	_on_off_power_factory_reset();
	power_cnt_reset = clock_time_ms();
	...
	while (1) {
		...
		main_loop();
		#if (RD_PIR_SENSOR)
			Pir_loop(PIR_LOOP_TIME,PIR_NOISE_CNT,PIR_FLAG_TIMEOUT,PIR_MOTION_TIMEOUT);
		#endif
		#if (RD_LIGHSENSOR)
			Lux_loop(LUX_LOOP_TIME);
		#endif
		flash_clear_power_cnt();
		rd_get_provision_state();
		checkMotion(motion_detected);
//		Join_confirm_check();
